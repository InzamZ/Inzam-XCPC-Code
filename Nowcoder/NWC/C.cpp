...
int solve() {
    ...
    for (int i = 1; i <= 25; ++i)
    {
        ...
        if (r != 2 && abs(b[i] * r - n) < ans)
        {
            ans = abs(b[i] * r - n);
            ansxy = make_pair(i, r);
        }
        if (l != 2 && abs(b[i] * l - n) < ans)
        {
            ans = abs(b[i] * l - n);
            ansxy = make_pair(i, l);
        }
