...
int solve() {
    ...
    for (int i = 0; i < v.size(); i++) {
        if (v[i].first > l) {
            ...
            l = max(l, v[i].second + 1);
        }
        else {
            ans += max(v[i].second - l + 1, 0ll);
            l = max(l, v[i].second + 1);
        }
    }
    cout << ans << '\n';
