...
int solve() {
    ...
    //             if (dp[j - 1] == 1e9)
    //                 continue;
    //             auto res = lower_bound(v[x].begin(), v[x].end(), dp[j - 1] + 1);
    //             if ( res != v[x].end()) {
    //                 dp[j] = min(dp[j], *res);
    //                 if (j == 2)
    //                     fir = s2[dp[j - 1]];
