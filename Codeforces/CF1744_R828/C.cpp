...
int solve() {
    ...
    for (int i = 0; i < s2.length(); ++i)
    {
        if (s2[i] == 'r') 
        {
            ...
            if (it != v.size())
                ans1 = max(ans1, v[it] - i);
        }
        if (s2[i] == 'y') 
        {
            auto it = lower_bound(v.begin(),v.end(),i)- v.begin();
            if (it != v.size())
                ans2 = max(ans2, v[it] - i);
        }
    }
    if (s1 == "r")
        cout << ans1 << endl;
