...
const int maxb = 110;
int T = 1, n, m, k, a[maxn], b[maxn];
map<int, pii>ans;
string s;

int digetsum(int num) {
    int res = 0;
    while (num) {
        res += num % 10;
        num /= 10;
...
int solve() {
    int bas = 1, ansl = 0, ansr = 0;
    cin >> n;
    while (n) {
        if (digetsum(ansl) < digetsum(ansr) )
            swap(ansl, ansr);
        ansl = ans[n % 1000].first * bas + ansl;
        ansr = ans[n % 1000].second * bas + ansr;
        bas *= 1000;
        n /= 1000;
    }
    cout << ansl << ' ' << ansr << '\n';
...
void main() {
    ...
    for (int i = 0; i <= 1000; ++i) {
        ...
        ansl = i / 2;
        ansr = i - ansl;
        while (abs(digetsum(ansl) - digetsum(ansr)) > 1) {
            ansl--; ansr++;
        }
        if (digetsum(ansl) > digetsum(ansr)) swap(ansl,ansr);
        ans[i] = {ansl, ansr};
    }
    cin >> T;
    while (T--) {
