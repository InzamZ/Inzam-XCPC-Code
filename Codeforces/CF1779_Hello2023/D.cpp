...
const int maxb = 110;
int T = 1, n, m, k, ans = 0, a[maxn], b[maxn], cnt[maxn];
vector<int>v;
string s;
...
int solve() {
    ...
    for (int i = 1; i <= m; ++i) {
        cin >> k;
        int idk = lower_bound(v.begin(), v.end(), k) - v.begin() + 1;
        if (v[idk - 1] == k)
            cnt[idk]++;
    }
    stack<int>st;
