...
int solve() {
    ...
    for (int i = n - 1; i >= 0; --i) {
        if (s[i] == '0') {
            char tmp = (s[i - 1] - '0') + (s[i - 2] - '0') * 10 + 'a' - 1;
            ans = tmp + ans;
            i -= 2;
        }
        else {
