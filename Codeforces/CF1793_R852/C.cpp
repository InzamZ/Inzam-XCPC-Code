...
int solve() {
    ...
    int l = 1, r = n;
    while (l <= r) {
        for (imn; imn <= n && b[imn]; ++imn);
        for (imx; b[imx] && imx > 0; --imx);
        if (a[l] == imn || a[l] == imx) {
            b[a[l]] = 1;
            l++;
        }
        else if (a[r] == imn || a[r] == imx) {
            b[a[r]] = 1;
            r--;
        }
        else
