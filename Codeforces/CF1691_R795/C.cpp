...
const int maxb = 110;
int T = 1, n, m, k, ans1 = 0, cnt1, ans2 = 0;
vector<int>v;
string s;
...
int solve() {
    ...
    sort(v.begin(), v.end());
    if (s[0] == '0' && s[n - 1] == '0' && cnt1 >= 2 && v[0] + (n - 1 - v[cnt1 - 1]) <= k)
        cout << (ans1 - 1) * 10 + (ans2 - 1) << '\n';
    else if (s[n - 1] == '0' && cnt1 >= 1 && (n - 1 - v[cnt1 - 1]) <= k && v[cnt1 - 1] == 0)
        cout << (ans1 - 1) * 10 + ans2 + 1 << '\n';
    else if (s[n - 1] == '0' && cnt1 >= 1 && (n - 1 - v[cnt1 - 1]) <= k)
        cout << (ans1 - 1) * 10 + ans2 << '\n';
    else if (s[0] == '0' && cnt1 >= 1 && v[0] <= k )
        cout << ans1 * 10 + ans2 - 1 << '\n';
    else
        cout << ans1 * 10 + ans2 << '\n';
    return 0;
