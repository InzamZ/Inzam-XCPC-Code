...
int solve() {
    ...
    for (int i = 1; i <= n; i++){
        ...
        while (l + 1 < r){
            ...
            for (int j = i; j <= n; ++j) {
                target_sum += mid + (i - j);
                // cout << "target_sum=" << target_sum << " j=" << j << " mid=" << mid << " a[j+1]=" << a[j + 1] << " j-i+1=" << j - i + 1 << endl;
