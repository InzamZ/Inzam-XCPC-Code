...
bool imap[maxn][maxn], vis[maxn][maxn];
string s;
...
void dfs(int x, int y) {
    vis[x + 1000][y + 1000] = 1;
    for (int i = 0; i < 6; ++i) {
        if (vis[x - dir[i][0] + 1000][y - dir[i][1] + 1000] == 0 && imap[x - dir[i][0] + 1000][y - dir[i][1] + 1000] == 1)
...
int solve() {
    ...
    for (int i = 1; i <= n ; ++i) {
        cin >> a[i] >> b[i];
        imap[a[i] + 1000][b[i] + 1000] = 1;
    }
    for (int i = 1; i <= n ; ++i) {
        if (vis[a[i] + 1000][b[i] + 1000] == 0) {
            ++ans; dfs(a[i], b[i]);
        }
    }
    cout << ans << '\n';
