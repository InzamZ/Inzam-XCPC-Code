...
int main()
{
    ...
    for (int i = 1; i <= q; ++i) {
        ...
        if (a == 1)
        {
            ...
            alpha[s[b]-'a']--;
            cnt[s[b]-'a'].add(b, -1);
            if (b!=1 && s[b] >= s[b-1] && s1[0] < s[b - 1]) ord.add(b-1,-1);
            if (b!=1 && s[b] < s[b-1] && s1[0] >= s[b - 1]) ord.add(b-1,1);
            if (s[b+1] >= s[b] && s[b+1] < s1[0]) ord.add(b,-1);
            if (s[b+1] < s[b] && s[b+1] >= s1[0]) ord.add(b,1);
            alpha[s1[0]-'a']++;
            cnt[s1[0]-'a'].add(b, -1);
            s[b] = s1[0];
            ...
            bool ans = 1;
            cin >> c;
            if (ord.sum(b, c) != c - b) ans = 0;
            // cout << "order " << ord.sum(b, c) << " " << c - b << '\n';
            for (int j = s[b] - 'a' + 1; j < s[c] - 'a'; ++j) {
                if (cnt[j].sum(b, c + 1) != alpha[j]) ans = 0;
